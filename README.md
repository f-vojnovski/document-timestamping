...
hash and the timestamp. The public key and the two algorithm names are not columns: they
describe the certificate in force rather than the stored row, so `ProofResponse` carries them
alongside the record on the way out.
...
are out of scope here and would be added before a real deployment. The schema is generated by
Hibernate from the entity, which suits a single table; a deployment with data worth keeping
would apply changes through versioned migrations instead.
...
document-timestamping-api/
  src/main/java/com/documenttimestamp/
    api/           DocumentController, ApiExceptionHandler, ApiErrorController,
                   CorsConfig
    service/       DocumentService, DocumentNotFoundException, TimestampingException
    repository/    DocumentRepository
    model/         Document, ProofResponse
