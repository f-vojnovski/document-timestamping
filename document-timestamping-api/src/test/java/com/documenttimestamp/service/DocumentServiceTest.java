...
class DocumentServiceTest {
    ...
    void storesAProofThatVerifiesWithThePublicKeyAlone() throws Exception {
        ProofResponse d = service.hashAndStoreDocument("Ducks research paper", file("hello world"));

        assertNotNull(d.encryptedHash());
        assertEquals(128, d.documentChecksum().length(), "SHA-512 is 64 bytes, 128 hex chars");
        assertEquals(128, d.targetHash().length());
        assertEquals(CipherUtility.SIGNATURE_ALGORITHM, d.signatureAlgorithm());
        assertEquals(DocumentService.HASHING_ALGORITHM, d.hashingAlgorithm());

        // What the standalone verifier does: check the signature over targetHash.
        assertTrue(cipherUtility.verifyDocumentHash(
                BytesHexConverter.hexStringToByteArray(d.targetHash()),
                BytesHexConverter.hexStringToByteArray(d.encryptedHash())));
    }
    ...
    void targetHashIsReproducibleOfflineFromTheDocumentAndTimestamp() throws Exception {
        ...
        buffer.write(ByteBuffer.allocate(Long.BYTES).putLong(d.timestamp()).array());
        byte[] recomputed = MessageDigest.getInstance("SHA-512").digest(buffer.toByteArray());

        assertEquals(d.documentChecksum(), BytesHexConverter.bytesToHex(checksum));
        assertEquals(d.targetHash(), BytesHexConverter.bytesToHex(recomputed));
    }
    ...
    void aModifiedDocumentNoLongerMatchesTheProof() throws Exception {
        ...
        assertNotEquals(d.documentChecksum(), BytesHexConverter.bytesToHex(tamperedChecksum));
    }
    ...
    void verifyFindsAPreviouslyTimestampedDocument() throws Exception {
        ...
        ProofResponse found = service.verifyDocument(file("hello world"));
        assertEquals("Ducks research paper", found.title());
        assertNotNull(found.publicKey());
    }
